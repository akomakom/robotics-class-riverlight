...
// unclear why in this sketch the values are different.
#define ANGLE_STRAIGHT 60 // 90 if the servo is perfectly mounted
#define ANGLE_LEFT     130 // these left/right values seem to get us about 180 degrees
...
void setup() {
  ...
  set_servo_angle(ANGLE_STRAIGHT);  //Servo rotates to 90°
...
void loop() {
  ...
  if (ir_rec == 0xFF6897) // button 1
  {
    set_servo_angle(ANGLE_LEFT);
  } 
  else if (ir_rec == 0xFFB04F) // button 3
  {
    set_servo_angle(ANGLE_RIGHT);
  }
  else if (ir_rec == 0xFF9867) // button 2
  {
